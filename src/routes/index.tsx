...
import { getCurrentPoster } from "@/lib/content.functions";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Bold Bible Talk — Thursdays at Rosebank Mall, Mochachos 18:30" },
      {
        name: "description",
        content:
          "Join Bold Bible Talk every Thursday at 18:30 — Mochachos, Rosebank Mall. Open, honest discussion of the Bible. Book a free one-on-one Bible study.",
      },
      { property: "og:title", content: "Bold Bible Talk — Thursdays at Rosebank Mall" },
      {
        property: "og:description",
        content: "Weekly Bible discussion · Thursdays 18:30 · Mochachos, Rosebank Mall.",
      },
...
function Index() {
  const fetchPoster = useServerFn(getCurrentPoster);
  const { data: poster } = useQuery({
    queryKey: ["current-poster"],
    queryFn: () => fetchPoster(),
  });

  const posterSrc = poster?.poster_url || fallbackPoster;
  const upcomingTitle = poster?.title;
  const upcomingDate = poster?.talk_date;
  const upcomingDescription = (poster as any)?.description as string | undefined;
  const upcomingTime = ((poster as any)?.start_time as string | undefined)?.slice(0, 5) || "18:30";
    ...
    <main className="min-h-screen bg-background text-foreground">
        ...
        <div className="relative mx-auto grid max-w-6xl gap-10 px-6 py-16 sm:py-24 md:grid-cols-2 md:items-center">
          <div className="text-primary-foreground">
            ...
            <h1 className="mt-5 font-serif text-4xl font-semibold leading-tight sm:text-5xl md:text-6xl">
              Open the Bible. <span className="text-[var(--gold)]">Ask anything.</span>
            </h1>
            {upcomingTitle ? (
            ...
            <div className="mt-8 flex flex-col gap-3 text-primary-foreground/90 sm:flex-row sm:flex-wrap sm:gap-x-6">
              ...
              <span className="flex items-center gap-2"><Clock className="h-5 w-5 text-[var(--gold)]" /> {upcomingTime}</span>
      ...
      <footer className="border-t border-border bg-background py-12">
        <div className="mx-auto flex max-w-6xl flex-col items-center gap-6 px-6 text-center">
          <img src={logo} alt="Bold Bible Talk" className="h-12 w-12 rounded-md" />
